      ...
      class Tower {
        ...
        public Tower(int xCoordinate, int yCoordinate, String towerType, Map Map){
          ...
          towerRange = Math.pow((range/2),2); // Range is the diameter of the tower's firing range
        ...
        // Checks whether enough time has elapsed for the tower to fire again
        void checkCooldown(GameTracker Game){
          // Check if the target exists, is alive, and is within the tower range
          if(Target != null && Target.health > 0 && checkDistance(Target.xPos, Target.yPos) <= towerRange){ 
        ...
        void fireProjectile(GameTracker Game){
          timeSinceLastFired = System.currentTimeMillis(); // Cooldown for firing 
          int targetX = Target.xPos;
          int targetY = Target.yPos;
          double distanceToTarget = Math.pow(checkDistance(targetX, targetY),0.5); // checkDistance returns distance squared
          ...
          // Add a new Projectile object to projectileList for the tower to fire the projectile
          Game.PTracker.projectileList.add(new Projectile(this.xPos, this.yPos, xVel, yVel, damage, projectileSize));
        }
        
        // Targets the enemy which is the farthest down the path and closest to the end of the path
        void findNewTarget(GameTracker Game){ 
          Enemy tempTarget;
          ...
          for(int i = 0; i < Game.ETracker.enemyList.size(); i++){
            if(checkDistance(Game.ETracker.enemyList.get(i).xPos, Game.ETracker.enemyList.get(i).yPos) <= towerRange){
        ...
        double checkDistance(int targetX, int targetY){ // Finds the distance from a target squared
